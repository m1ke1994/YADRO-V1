...
Production-адрес: `http://95.31.22.162`.
...
DJANGO_ALLOWED_HOSTS=95.31.22.162
...
DOMAIN=95.31.22.162
SITE_BASE_URL=http://95.31.22.162
PUBLIC_BASE_URL=http://95.31.22.162
FRONTEND_URL=http://95.31.22.162
API_URL=http://95.31.22.162/api
ADMIN_URL=http://95.31.22.162/admin

CORS_ALLOW_ALL_ORIGINS=False
CORS_ALLOWED_ORIGINS=http://95.31.22.162
CORS_ALLOWED_ORIGIN_REGEXES=
CSRF_TRUSTED_ORIGINS=http://95.31.22.162
...
VITE_FAVICON_URL=http://95.31.22.162/favicon.svg
VITE_OG_IMAGE_URL=http://95.31.22.162/og-image.svg
VITE_API_BASE_URL=http://95.31.22.162
```
...
`95.31.22.162` откройте входящие TCP-порты `22` и `80`.
...
- все URL и origin для `http://95.31.22.162`.
...
Nginx принимает HTTP на `95.31.22.162:80`. Proxy передаёт Django заголовки `Host`,
...
```bash
curl -X POST http://95.31.22.162/api/auth/token/ \
...
```bash
curl -X POST http://95.31.22.162/api/leads/ \
  -H "Content-Type: application/json" \
...
```text
http://95.31.22.162/api/public/telegram/webhook/
```
...
```bash
curl -I http://95.31.22.162/robots.txt
curl -I http://95.31.22.162/sitemap.xml
curl -I http://95.31.22.162/favicon.svg
curl -I http://95.31.22.162/og-image.svg
curl -s http://95.31.22.162/ | grep -E "canonical|og:|description"
```
...
```bash
curl -I http://95.31.22.162/some-route
curl -I http://95.31.22.162/dashboard
```
...
```bash
curl -I http://95.31.22.162/
curl -I http://95.31.22.162/dashboard
curl -I http://95.31.22.162/admin/
curl -I http://95.31.22.162/static/admin/css/base.css
curl -I http://95.31.22.162/robots.txt
curl -I http://95.31.22.162/sitemap.xml
curl -I http://95.31.22.162/tracker.js
curl http://95.31.22.162/api/health/
```
